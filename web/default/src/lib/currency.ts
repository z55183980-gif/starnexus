...
/**
 * Payment gateway currency metadata.
 *
 * Epay / Alipay / WeChat amounts returned by the top-up API are priced via
 * operation_setting.Price in the payment gateway currency (typically CNY),
 * which is independent of quota display settings.
 */
function getStripeSettlementDisplayMeta(
  stripeCurrency?: string
): DisplayMeta {
  switch ((stripeCurrency || 'usd').toLowerCase()) {
    case 'cny':
      return {
        kind: 'currency',
        symbol: '¥',
...
function getPaymentGatewayDisplayMeta(
  config: CurrencyConfig,
  paymentType?: string,
  stripeCurrency?: string
): DisplayMeta {
  if (paymentType === 'stripe') {
    return getStripeSettlementDisplayMeta(stripeCurrency)
  }
...
export interface PaymentGatewayFormatOptions extends CurrencyFormatOptions {
  paymentType?: string
  stripeCurrency?: string
}

/**
 * Format a payment charge amount returned by top-up APIs.
 *
 * Uses payment gateway currency (typically CNY for epay) rather than quota
 * display currency, so ¥71 is shown correctly even when quota is displayed as USD.
 */
export function formatPaymentGatewayAmount(
  amount: number | null | undefined,
  options?: PaymentGatewayFormatOptions
): string {
  if (amount == null || Number.isNaN(amount)) return '-'

  const { config } = getCurrencyDisplay()
  const meta = getPaymentGatewayDisplayMeta(
    config,
    options?.paymentType,
...
/**
 * Short label for payment gateway currency in UI copy.
 */
export function getPaymentGatewayCurrencyLabel(
  paymentType?: string,
  stripeCurrency?: string
): string {
  if (paymentType === 'stripe') {
    const meta = getStripeSettlementDisplayMeta(stripeCurrency)
    return meta.kind === 'currency' ? meta.currencyCode : 'USD'
  }
  if (paymentType === 'usdt') {
    return 'USDT'
  }
  ...
  const { config } = getCurrencyDisplay()
  const meta = getPaymentGatewayDisplayMeta(config, paymentType)
  if (meta.kind === 'currency') {
