...
export function CommonLogsFilterBar<TData>(
  ...
  useEffect(() => {
    ...
    if (searchParams.excludeFilters?.length) {
      next.excludeFilters = searchParams.excludeFilters
    }
  ...
  const handleApply = useCallback(() => {
    const sanitizedExcludeFilters = (filters.excludeFilters || []).filter(
      (item) => item.value.trim() !== ''
    )
    const filterParams = buildSearchParams(
      {
        ...filters,
        excludeFilters: sanitizedExcludeFilters,
      },
      'common'
    )
    navigate({
  ...
  const hasActiveExcludeFilters = (filters.excludeFilters || []).some(
    (item) => item.value.trim() !== ''
  )
  ...
  const hasSecondaryFilters =
    ...
    (filters.excludeFilters?.length ?? 0) > 0

  const hasAdditionalFilters =
    !!filters.model || !!filters.group || !!logType || hasSecondaryFilters
      ...
      secondarySearch={
        <>
          ...
          <Input
            ...
            className={inputClass}
          />
          <ExcludeFiltersEditor
            filters={filters.excludeFilters || []}
            onChange={(excludeFilters) =>
              setFilters((prev) => ({ ...prev, excludeFilters }))
            }
            isAdmin={isAdmin}
