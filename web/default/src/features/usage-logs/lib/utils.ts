...
function serializeExcludeFilters(
  searchParams: Record<string, unknown>
): string | undefined {
  const raw = searchParams.excludeFilters
  if (!Array.isArray(raw) || raw.length === 0) {
    return undefined
  }

  const valid = raw.filter((item): item is LogExcludeFilter => {
    if (typeof item !== 'object' || item === null) {
      return false
    }
    const field = (item as LogExcludeFilter).field
    const value = (item as LogExcludeFilter).value
    return (
      (field === 'model_name' ||
        field === 'token_name' ||
        field === 'group' ||
...
export async function fetchLogsByCategory(
  ...
  if (logCategory === 'common') {
    const params = buildApiParams({
      ...
      searchParams,
      columnFilters,
      isAdmin: isManagedScope,
    })
    if (accessScope === 'admin') return await getAllLogs(params)
    if (accessScope === 'agent') return await getAgentLogs(params)
