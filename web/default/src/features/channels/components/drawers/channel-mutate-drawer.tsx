...
export function ChannelMutateDrawer({
  ...
  // Validate base_url - warn if it includes API version paths that are appended automatically
  useEffect(() => {
    if (!currentBaseUrl) return

    const trimmed = currentBaseUrl.replace(/\/+$/, '')
    const needsWarning =
      trimmed.endsWith('/v1') ||
      (currentType === 26 &&
        (trimmed.endsWith('/v4') ||
          trimmed.includes('/api/paas/v4') ||
          trimmed.includes('/api/paas/v3'))) ||
      (currentType === 16 && trimmed.includes('/api/paas/'))

    if (!needsWarning) return

    const timer = setTimeout(() => {
      toast.warning(
        currentType === 26 || currentType === 16
          ? t(
              'Warning: For Zhipu channels, use the domain only (e.g. https://open.bigmodel.cn). Do not include /api/paas/v4 or /v4 — paths are added automatically.'
            )
          : t(
