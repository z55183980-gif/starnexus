...
function isDashboardAnnouncement(item: AnnouncementItem) {
  return !item.target || item.target === 'dashboard' || item.target === 'both'
}
...
export function useAnnouncements() {
  ...
  return {
    ...result,
    items: result.items.filter(isDashboardAnnouncement),
  }
}
