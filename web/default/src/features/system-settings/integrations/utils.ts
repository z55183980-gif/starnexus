...
export function resolveEpusdtGatewayAddress(value: string): string {
  return removeTrailingSlash(value) || 'https://pay.xingyuapi.com'
}

export function isLegacyEpusdtGatewayAddress(
  gatewayAddress: string,
  legacyPayAddress: string
): boolean {
  const gateway = removeTrailingSlash(gatewayAddress)
  const payAddress = removeTrailingSlash(legacyPayAddress)
  return gateway !== '' && payAddress !== '' && gateway === payAddress
}

export function isEpayConfigured(settings: {
  PayAddress?: string
  EpayId?: string
  EpayKey?: string
}): boolean {
  return [settings.PayAddress, settings.EpayId, settings.EpayKey].every(
    (value) => Boolean(value?.trim())
  )
}

const MODERN_PAY_METHOD_TYPES = new Set([
  'stripe',
  'usdt',
  'waffo',
  'waffo_pancake',
...
function parsePayMethodsJson(payMethodsJson: string): Array<{ type?: string; enabled?: string }> {
  const trimmed = payMethodsJson.trim()
  if (!trimmed) return []
  try {
    const parsed: unknown = JSON.parse(trimmed)
    return Array.isArray(parsed) ? (parsed as Array<{ type?: string; enabled?: string }>) : []
  } catch {
    return []
  }
}

export function isLegacyEpayOfferedInPayMethods(payMethodsJson: string): boolean {
  return parsePayMethodsJson(payMethodsJson).some((method) => {
    const type = method.type?.trim() ?? ''
    if (!type || MODERN_PAY_METHOD_TYPES.has(type)) {
      return false
    }
...
export function isLegacyEpusdtGatewayConflict(
  gatewayAddress: string,
  legacyPayAddress: string,
  epayConfigured: boolean,
  payMethodsJson = ''
): boolean {
  if (!epayConfigured || !isLegacyEpayOfferedInPayMethods(payMethodsJson)) {
    return false
  }
