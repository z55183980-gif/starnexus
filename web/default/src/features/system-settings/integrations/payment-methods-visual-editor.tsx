...
export function PaymentMethodsVisualEditor({
  ...
  const filteredMethods = useMemo(() => {
    ...
    return paymentMethods.filter(
      (method) =>
        ...
        (method.tag || '').toLowerCase().includes(lowerSearch)
    )
  ...
  const handleSave = (data: PaymentMethodData) => {
    ...
    onChange(normalizePayMethodsJson(JSON.stringify(updatedArray, null, 2)))
  }
  ...
  const handleInsertTemplate = (template: PaymentMethodData) => {
    ...
    if (!exists) {
      ...
      onChange(normalizePayMethodsJson(JSON.stringify(parsed, null, 2)))
    }
  }
                ...
                {filteredMethods.map((method, index) => {
                  ...
                  return (
                    <TableRow key={`${method.type}-${index}`}>
                      ...
                      <TableCell>
                        {method.enabled === 'false' ? t('Off') : t('On')}
                      </TableCell>
                      <TableCell>
                        {method.tag ? (
