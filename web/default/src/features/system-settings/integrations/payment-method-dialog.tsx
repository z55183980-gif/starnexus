...
export function PaymentMethodDialog({
  ...
  const handleSubmit = (values: PaymentMethodDialogFormValues) => {
    ...
    data.enabled = values.enabled ? 'true' : 'false'
    if (values.tag && values.tag.trim() !== '') {
      data.tag = values.tag.trim()
    }
    onSave(data)
    form.reset()
    onOpenChange(false)
              ...
              render={({ field }) => (
                <FormItem className='flex flex-row items-center justify-between rounded-lg border p-3'>
                  <div className='space-y-0.5'>
                    ...
                    <FormDescription>
                      {t('Toggle this payment method visibility in wallet.')}
                    </FormDescription>
                  </div>
                  <FormControl>
                    <Switch
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder={t('e.g., Recommended, Fast')} {...field} />
                  </FormControl>
                  <FormDescription>
                    {t('Shown as a small label on wallet payment button.')}
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
