...
const RATIO_DRAFT_PATTERN = /^\d*\.?\d*$/

function isValidRatioDraft(raw: string): boolean {
  return RATIO_DRAFT_PATTERN.test(raw)
}

// Returns null for empty or incomplete decimals (e.g. "", ".", "0.") so typing is not truncated.
...
function normalizeRatio(value: unknown): number {
  ...
  const parsed = parseRatioInput(String(value))
...
export const GroupRatioVisualEditor = memo(function GroupRatioVisualEditor({
  ...
  const handleSimpleSave = (name: string, value: string) => {
    ...
    map[name] = parseRatioInput(value) ?? 0
...
function GroupPricingTable({
  ...
  const emitRows = useCallback(
    (nextRows: GroupPricingRow[]) => {
      ...
      lastEmittedSignatureRef.current = sourceGroupPricingSignature(
  ...
  const handleRatioChange = useCallback(
    (id: string, raw: string) => {
      if (!isValidRatioDraft(raw)) {
        return
      }
      setRatioDrafts((current) => ({ ...current, [id]: raw }))
      const parsed = parseRatioInput(raw)
      if (parsed !== null) {
        updateRow(id, 'ratio', parsed)
      }
    },
    [updateRow]
  )

  const commitRatioDraft = useCallback(
    (id: string) => {
      const draft = ratioDrafts[id]
      if (draft === undefined) {
        return
      }
                    ...
                    <TableRow key={row._id}>
                      ...
                      <TableCell>
                        <Input
                          ...
                          inputMode='decimal'
                          value={
                            ratioDrafts[row._id] ?? formatRatioValue(row.ratio)
                          }
                          onChange={(event) =>
                            handleRatioChange(row._id, event.target.value)
                          }
                          onBlur={() => commitRatioDraft(row._id)}
                        />
                      </TableCell>
                      <TableCell>
...
function SimpleGroupDialog({
  ...
  const handleSave = () => {
    ...
    if (parseRatioInput(value) === null) return
...
function GroupOverrideDialog({
  ...
  const handleSave = () => {
    ...
    const parsedRatio = parseRatioInput(ratio)
