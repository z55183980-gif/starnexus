...
export function PerformanceSection(props: Props) {
  ...
  const onSubmit = async (values: PerfFormValues) => {
    ...
    // Keep the current parent snapshot as the synchronization marker. The
    // invalidated options query may still expose the old values for one
    // render; marking the stale snapshot here prevents that render from
    // resetting the just-saved form before the refetch delivers new values.
    baselineSerializedRef.current = JSON.stringify(props.defaultValues)
