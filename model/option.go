...
func clampAffiliateOptionRate(value string) float64 {
...
func InitOptionMap() {
	...
	common.OptionMap["ApiBaseUrl"] = "https://api.xingyuapi.com"
	...
	common.OptionMap["GPTImagePrice"] = ratio_setting.GPTImagePrice2JSONString()
	...
	common.OptionMap["VideoTokenPrice"] = ratio_setting.VideoTokenPrice2JSONString()
...
func loadOptionsFromDatabase() {
	...
	sanitizeLegacyEpUSDTSettings()
	syncPaymentSecretConfiguredFlags()
}

func sanitizeLegacyEpUSDTSettings() {
	if !setting.IsLegacyEpUSDTGatewayConflict(
		setting.EpUSDTGatewayAddress,
		operation_setting.PayAddress,
	) {
		return
	}

	common.SysLog("clearing legacy EpUSDT gateway address that matches PayAddress")
	setting.EpUSDTGatewayAddress = ""
	common.OptionMapRWMutex.Lock()
	common.OptionMap["EpUSDTGatewayAddress"] = ""
	common.OptionMapRWMutex.Unlock()

	if err := UpdateOption("EpUSDTGatewayAddress", ""); err != nil {
		common.SysLog("failed to clear legacy EpUSDT gateway address: " + err.Error())
	}
}
...
func updateOptionMap(key string, value string) (err error) {
	...
	case "SupportChannels":
		err = setting.UpdateSupportChannelsByJsonString(value)
	case "AutoGroups":
	...
	case "StripeCurrency":
		setting.StripeCurrency = setting.NormalizeStripeCurrency(value)
	...
	case "EpUSDTGatewayAddress":
		setting.EpUSDTGatewayAddress = value
	case "EpUSDTApiToken":
	...
	case "AffiliateRebateFreezeHours":
		...
		if common.AffiliateRebateFreezeHours < 0 {
			common.AffiliateRebateFreezeHours = 0
		}
		if common.AffiliateRebateFreezeHours > common.AffiliateRebateFreezeHoursMax {
			common.AffiliateRebateFreezeHours = common.AffiliateRebateFreezeHoursMax
		}
	case "AffiliateRebateDurationDays":
		common.AffiliateRebateDurationDays, _ = strconv.Atoi(value)
		if common.AffiliateRebateDurationDays < 0 {
			common.AffiliateRebateDurationDays = 0
		}
		if common.AffiliateRebateDurationDays > common.AffiliateRebateDurationDaysMax {
			common.AffiliateRebateDurationDays = common.AffiliateRebateDurationDaysMax
		}
	case "AffiliateRebatePerInviteeCapUSD":
		common.AffiliateRebatePerInviteeCapUSD, _ = strconv.ParseFloat(value, 64)
		if common.AffiliateRebatePerInviteeCapUSD < 0 {
			common.AffiliateRebatePerInviteeCapUSD = 0
		}
	...
	case "GPTImagePrice":
		err = ratio_setting.UpdateGPTImagePriceByJSONString(value)
	case "CacheRatio":
	...
	case "VideoTokenPrice":
		err = ratio_setting.UpdateVideoTokenPriceByJSONString(value)
	case "TopUpLink":
	...
	if err == nil {
		switch key {
		case "ModelRatio", "ModelPrice", "CompletionRatio", "CacheRatio", "CreateCacheRatio", "ImageRatio", "AudioRatio", "AudioCompletionRatio", "GPTImagePrice", "VideoTokenPrice":
			InvalidatePricingCache()
		}
	}

	switch key {
	case "StripeApiSecret", "StripeWebhookSecret", "EpUSDTApiToken":
		syncPaymentSecretConfiguredFlagsLocked()
	}
...
func syncPaymentSecretConfiguredFlagsLocked() {
	common.OptionMap["StripeApiSecretConfigured"] = strconv.FormatBool(isStripeApiSecretConfigured())
	common.OptionMap["StripeWebhookSecretConfigured"] = strconv.FormatBool(strings.TrimSpace(setting.StripeWebhookSecret) != "")
	common.OptionMap["EpUSDTApiTokenConfigured"] = strconv.FormatBool(strings.TrimSpace(setting.EpUSDTApiToken) != "")
}

func syncPaymentSecretConfiguredFlags() {
	common.OptionMapRWMutex.Lock()
	defer common.OptionMapRWMutex.Unlock()
	syncPaymentSecretConfiguredFlagsLocked()
}
