...
// buildFuzzyLikePattern 将用户关键词转换为 LIKE 模糊匹配模式。
// 不含 % 时自动包裹为 %keyword%，含 % 时沿用 sanitizeLikePattern 规则。
func buildFuzzyLikePattern(input string) (string, error) {
	...
	if len(input) < 2 {
		return "", errors.New("搜索关键词长度至少为 2 个字符")
	}

	sanitized, err := sanitizeLikePattern(input)
	if err != nil {
		return "", err
	}
