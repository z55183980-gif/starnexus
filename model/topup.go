...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	ProcessAffiliateRebateAfterTopUp(topUp)
...
func applyTopUpKeywordFilter(query *gorm.DB, keyword string, includeUserID bool) (*gorm.DB, error) {
	keyword = strings.TrimSpace(keyword)
	if keyword == "" {
		return query, nil
	}

	pattern, err := buildFuzzyLikePattern(keyword)
	if err != nil {
		return nil, err
	}

	if includeUserID {
		if userID, convErr := strconv.Atoi(keyword); convErr == nil {
			return query.Where(
				"(trade_no LIKE ? ESCAPE '!' OR user_id = ?)",
				pattern,
				userID,
			), nil
		}

		normalized := NormalizeUsername(keyword)
		var userIds []int
		if err := DB.Model(&User{}).Where("username = ?", normalized).Pluck("id", &userIds).Error; err != nil {
			return nil, err
		}
		if len(userIds) > 0 {
			return query.Where(
				"(trade_no LIKE ? ESCAPE '!' OR user_id IN ?)",
				pattern,
				userIds,
			), nil
		}
...
// FillTopUpUsernames attaches usernames to top-up records for admin display.
func FillTopUpUsernames(topups []*TopUp) {
	if len(topups) == 0 {
		return
	}

	userIdMap := make(map[int]*UserBase)
	for _, topUp := range topups {
		if _, ok := userIdMap[topUp.UserId]; ok {
			continue
		}
		cacheUser, err := GetUserCache(topUp.UserId)
		if err == nil {
			userIdMap[topUp.UserId] = cacheUser
		}
	}

	for _, topUp := range topups {
		if user, ok := userIdMap[topUp.UserId]; ok {
...
func SearchUserTopUps(userId int, keyword string, pageInfo *common.PageInfo) (topups []*TopUp, total int64, err error) {
	...
	var perr error
	query, perr = applyTopUpKeywordFilter(query, keyword, false)
	if perr != nil {
		tx.Rollback()
		return nil, 0, perr
	}
...
func ManualCompleteTopUp(tradeNo string, callerIp string) error {
	...
	if completed := GetTopUpByTradeNo(tradeNo); completed != nil {
		ProcessAffiliateRebateAfterTopUp(completed)
	}
	return nil
}
...
func RechargeEpusdt(tradeNo string, callerIp string) (err error) {
	...
	if quotaToAdd > 0 {
		RecordTopupLog(topUp.UserId, fmt.Sprintf("EpUSDT充值成功，充值额度: %v，支付金额: %.2f", logger.FormatQuota(quotaToAdd), topUp.Money), callerIp, topUp.PaymentMethod, PaymentMethodUsdt)
		ProcessAffiliateRebateAfterTopUp(topUp)
	}
...
func MarkTopUpExpired(tradeNo string, expectedProvider string) error {
	if tradeNo == "" {
		return errors.New("未提供支付单号")
	}
