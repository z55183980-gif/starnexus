...
type User struct {
	...
	DoubaoVideo2MaterialBytes      int64          `json:"-" gorm:"not null;default:0;column:doubao_video2_material_bytes"`
	DoubaoVideo2MaterialLimitBytes int64          `json:"-" gorm:"not null;default:0;column:doubao_video2_material_limit_bytes"` // 0 uses the system default
	RequestCount                   int            `json:"request_count" gorm:"type:int;default:0;"`                              // request number
	Concurrency                    int            `json:"concurrency" gorm:"type:int;default:5"`                                 // max in-flight relay requests, <=0 means unlimited
...
func CheckUserExistOrDeleted(username string, email string) (bool, error) {
	...
	if email == "" {
		...
		err = DB.Unscoped().First(&user, "LOWER(username) = LOWER(?) OR email = ?", username, email).Error
	}
	if err != nil {
...
func GetUserIdByAffCode(affCode string) (int, error) {
	...
	if record, err := GetUserAffiliateByCode(affCode); err == nil && record.UserId > 0 {
		return record.UserId, nil
	}
	var user User
	err := DB.Select("id").Where("UPPER(aff_code) = ?", affCode).First(&user).Error
	if err != nil {
		return 0, err
	}
	return user.Id, nil
...
func (user *User) Insert(inviterId int) error {
	...
	if _, err := ensureUserAffiliateRecord(user.Id, user.AffCode); err != nil {
		common.SysLog(fmt.Sprintf("ensure user affiliate failed user_id=%d err=%v", user.Id, err))
	}
	bindAffiliateInviterAfterRegistration(user.Id, inviterId)
...
func (user *User) FinalizeOAuthUserCreation(inviterId int) {
	...
	if err := DB.Where("id = ?", user.Id).First(&createdUser).Error; err == nil {
		if _, err := ensureUserAffiliateRecord(createdUser.Id, createdUser.AffCode); err != nil {
			common.SysLog(fmt.Sprintf("ensure user affiliate failed user_id=%d err=%v", createdUser.Id, err))
		}
		bindAffiliateInviterAfterRegistration(createdUser.Id, inviterId)
		defaultSidebarConfig := generateDefaultSidebarConfigForRole(createdUser.Role)
		if defaultSidebarConfig != "" {
			currentSetting := createdUser.GetSetting()
			currentSetting.SidebarModules = defaultSidebarConfig
...
func (user *User) ValidateAndFill() (err error) {
	...
	err = DB.Where("LOWER(username) = LOWER(?) OR email = ?", username, username).First(user).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
...
// UpdateUserUsedQuota adjusts usage accounting without counting a new request.
// Async task settlement uses this for post-completion billing deltas because
// the original submission already incremented request_count.
