...
func GetStatus(c *gin.Context) {
	...
	data := gin.H{
		...
		"chats":                         setting.Chats,
		"support_channels":              setting.SupportChannels,
...
func SendEmailVerification(c *gin.Context) {
	...
	subject := fmt.Sprintf("%s 邮箱验证 / Email Verification", common.SystemName)
	content := fmt.Sprintf(
		"<p>您好，您正在进行 <strong>%s</strong> 的邮箱验证。</p>"+
			"<p>您的验证码为: <strong>%s</strong></p>"+
			"<p>验证码 %d 分钟内有效。如非本人操作，请忽略本邮件。</p>"+
			`<hr style="border:none;border-top:1px solid #e5e5e5;margin:24px 0;" />`+
			"<p>Hello, you are verifying your email for <strong>%s</strong>.</p>"+
...
func SendPasswordResetEmail(c *gin.Context) {
	...
	if model.IsEmailAlreadyTaken(email) {
		...
		subject := fmt.Sprintf("%s 密码重置 / Password Reset", common.SystemName)
		content := fmt.Sprintf(
			"<p>您好，您正在进行 <strong>%s</strong> 的密码重置。</p>"+
				"<p>请点击下方链接重置密码：<br><a href='%s'>点此重置密码</a></p>"+
				"<p>若按钮无法点击，请复制以下链接到浏览器打开：<br>%s</p>"+
				"<p>重置链接 %d 分钟内有效。如非本人操作，请忽略本邮件。</p>"+
				`<hr style="border:none;border-top:1px solid #e5e5e5;margin:24px 0;" />`+
				"<p>Hello, you are resetting your password for <strong>%s</strong>.</p>"+
				"<p>Please click the link below to reset your password:<br><a href='%s'>Reset password</a></p>"+
				"<p>If the link does not work, copy and paste this URL into your browser:<br>%s</p>"+
				"<p>This link is valid for %d minutes. If you did not request this, please ignore this email.</p>",
