...
func GetTopUpInfo(c *gin.Context) {
	...
	if isStripeTopUpEnabled() {
		...
		payMethods = removePayMethodType(payMethods, model.PaymentMethodStripe)
	}

	enableUsdt := isUsdtTopUpEnabled()
	if enableUsdt {
		if !hasPayMethodType(payMethods, paymentMethodUsdt) {
		...
		payMethods = removePayMethodType(payMethods, paymentMethodUsdt)
	}
	...
	data := gin.H{
		...
		"stripe_currency":         setting.StripeSettlementCurrency(),
...
func GetAllTopUps(c *gin.Context) {
	...
	model.FillTopUpUsernames(topups)
...
func hasPayMethodType(payMethods []map[string]string, methodType string) bool {
	for _, method := range payMethods {
...
func removePayMethodType(payMethods []map[string]string, methodType string) []map[string]string {
	if methodType == "" {
		return payMethods
	}
