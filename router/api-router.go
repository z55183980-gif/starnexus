...
func SetApiRouter(router *gin.Engine) {
	...
	apiRouter.Use(gzip.Gzip(
		...
		gzip.WithExcludedPaths([]string{"/api/business-monitor/stream"}),
	))
	...
	apiRouter.Use(middleware.GlobalAPIRateLimit())
	{
		...
		apiRouter.GET("/status/node", middleware.AdminAuth(), controller.GetCurrentNode)
		...
		apiRouter.POST("/epusdt/notify", controller.EpusdtNotify)
		...
		{
			userRoute.POST("/register", middleware.AuthRateLimit("RG"), middleware.TurnstileCheck(), controller.Register)
			userRoute.POST("/login", middleware.AuthRateLimit("LG"), middleware.TurnstileCheck(), controller.Login)
			userRoute.POST("/login/2fa", middleware.AuthRateLimit("L2"), controller.Verify2FALogin)
			userRoute.POST("/passkey/login/begin", middleware.AuthRateLimit("PB"), controller.PasskeyLoginBegin)
			userRoute.POST("/passkey/login/finish", middleware.AuthRateLimit("PF"), controller.PasskeyLoginFinish)
			...
			selfRoute.Use(middleware.UserAuth())
			{
				...
				selfRoute.GET("/affiliate", controller.GetAffiliateDetail)
				selfRoute.POST("/affiliate/transfer", controller.TransferAffiliateUSD)
				...
				selfRoute.GET("/login-announcements/unread", controller.GetUnreadLoginAnnouncements)
				selfRoute.POST("/login-announcements/read", controller.MarkLoginAnnouncementsRead)
		...
		nodeRoutingRoute.Use(middleware.AdminAuth())
		{
			...
			nodeRoutingRoute.GET("/alert-rules", controller.GetRoutingNodeAlertRules)
			nodeRoutingRoute.GET("/alerts", controller.GetRoutingNodeAlerts)
			nodeRoutingRoute.GET("/alerts/unread", controller.GetRoutingNodeAlertUnreadSummary)
			nodeRoutingRoute.POST("/alerts/read", controller.MarkRoutingNodeAlertsRead)
			nodeRoutingRoute.GET("/alerts/:id/events", controller.GetRoutingNodeAlertEvents)
			nodeRoutingRoute.POST("/alerts/:id/acknowledge", middleware.RootAuth(), controller.AcknowledgeRoutingNodeAlert)
			nodeRoutingRoute.POST("/alerts/:id/silence", middleware.RootAuth(), controller.SilenceRoutingNodeAlert)
			nodeRoutingRoute.POST("/alerts/:id/resolve", middleware.RootAuth(), controller.ResolveRoutingNodeAlert)
		...
		upstreamRoute.Use(middleware.RootAuth())
		{
			...
			upstreamRoute.GET("/accounts/:id/today-stats", controller.GetUpstreamAccountTodayStats)
			upstreamRoute.POST("/accounts/today-stats/batch", controller.GetUpstreamAccountTodayStatsBatch)
			upstreamRoute.GET("/accounts/:id/scheduled-tests", controller.ListUpstreamAccountScheduledTestPlans)
			upstreamRoute.POST("/accounts/:id/scheduled-tests", controller.CreateUpstreamAccountScheduledTestPlan)
			upstreamRoute.PUT("/accounts/:id/scheduled-tests/:planId", controller.UpdateUpstreamAccountScheduledTestPlan)
			upstreamRoute.DELETE("/accounts/:id/scheduled-tests/:planId", controller.DeleteUpstreamAccountScheduledTestPlan)
			upstreamRoute.GET("/accounts/:id/scheduled-tests/:planId/results", controller.ListUpstreamAccountScheduledTestResults)
			...
			upstreamRoute.DELETE("/proxies/:id/real-failures/:eventId", controller.DeleteUpstreamProxyRealFailure)
		...
		optionRoute.Use(middleware.RootAuth())
		{
			...
			optionRoute.PUT("/codex", controller.UpdateCodexSettings)
			optionRoute.POST("/codex/version-sync", controller.SyncCodexVersion)
		...
		securityAuditRoute.Use(middleware.RootAuth())
		{
			...
			securityAuditRoute.GET("/api-suspension-settings", controller.GetSecurityAuditBanConfig)
			securityAuditRoute.PUT("/api-suspension-settings", controller.UpdateSecurityAuditBanConfig)
		}
		...
		{
			affiliateAdminRoute.GET("/ledger", controller.AdminListAffiliateLedgers)
		}
		...
		tokenRoute.Use(middleware.UserAuth())
		{
			...
			tokenRoute.GET("/usage", controller.GetTokenUsageWindows)
		...
		// User-owned DoubaoVideo2.0 material library. Dashboard operations use
		...
		doubaoVideoRoute.Use(middleware.UserAuth())
		{
			...
			doubaoVideoRoute.DELETE("/access-keys/:id", middleware.CriticalRateLimit(), controller.DeleteDoubaoVideo2AccessKey)
		}
		...
		logRoute.GET("/summary", middleware.AdminAuth(), controller.GetLogsSummary)
