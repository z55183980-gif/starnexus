...
func TestResponsesStreamDataStartsClientOutput(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "encrypted reasoning item", data: `{"type":"response.output_item.added","item":{"type":"reasoning","encrypted_content":"cipher"}}`, want: false},
		{name: "encrypted reasoning item done", data: `{"type":"response.output_item.done","item":{"type":"reasoning","encrypted_content":"cipher"}}`, want: false},
		{name: "encrypted reasoning with visible summary", data: `{"type":"response.output_item.added","item":{"type":"reasoning","encrypted_content":"cipher","summary":[{"type":"summary_text","text":"thinking"}]}}`, want: true},
